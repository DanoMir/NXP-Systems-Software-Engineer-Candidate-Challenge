...
### Execution and Validation (Core Acceptance Criteria)
    ...
    1. Navigate to the execution directory: The directory simtemp/scripts/ contains all scripts of automation: build.sh and run_demo.sh
    ```bash
        cd simtemp/scripts
    ...
    NOTE: All this code was developed in VS Code, therefore there may be a problem when trying to execute bash code:
        run_demo.sh: line X: $'\r': command not found 
        This is an error CRLF between Windows(DOS) and Linux(Unix)

        To solve this error please check the section: ### Some issues resolve (* EOL format conflict in bash during run .sh files:)

    The run_demo.sh script automates the full test sequence (Load, Permissions, Test, Unload).

    A. Validation of Success (Default Test)
    This Test verifies the critical Alert Path (POLLPRI) functionality usig the default low threshold and report success or failure.
